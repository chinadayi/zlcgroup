...
<div id="wrap">
	<div class="tab">
		<ul>
			...
			<li><a href="javascript:void(0);" class="on">添加字段</a></li>
		</ul>
	</div>
	<div class="main">
	<fieldset>
	...
	<table cellspacing="0" class="sub" width="100%">
		...
		<tr>
			...
			<td>
			<input type="text" name="info[enname]"  id="enname" onblur="$.get('?mod=form&file=form&action=chkfieldname&formid=<?php echo $formid;?>',{data:this.value},function(data){if(data=='yes'){alert('该字段名已经存在，请更换!');$('#enname').val('');$('#enname').focus();}});" datatype="table" class="input" size="16" />
		...
		<tr class="bg2"><td></td><td><input type="button" class="button" onclick="doSubmit(this);" value="添加字段" /> <input type="button" class="button" onclick="window.location.href='?mod=form&file=form&action=fields&id=<?php echo $formid;?>'" value="取消返回" /></td></tr>
	</table>
	</form>
