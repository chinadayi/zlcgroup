...
* $email=new email();
* // 发送邮件
* $email->send('收件人账号','邮件主题','邮件内容','邮件来源');
*/
class email
{
	function email()
	...
	function send($email_to, $email_subject, $email_message, $email_from = '')
	{
		...
		$adminemail = $this->type == 1 ? $RETENG['mail_user'] : $RETENG['mail_user'];
		$email_from = $email_from == '' ? '=?'.CHARSET.'?B?'.base64_encode($RETENG['site_name'])."?= <$adminemail>" : (preg_match('/^(.+?) \<(.+?)\>$/',$email_from, $from) ? '=?'.CHARSET.'?B?'.base64_encode($from[1])."?= <$from[2]>" : $email_from);
		...
		$headers = "From: $email_from{$this->delimiter}X-Priority: 3{$this->delimiter}X-Mailer: RETENG {$this->delimiter}MIME-Version: 1.0{$this->delimiter}Content-type: text/html; charset=".CHARSET."{$this->delimiter}";
		if($this->type == 1)
		{
