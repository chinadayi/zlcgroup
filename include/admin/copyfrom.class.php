...
class copyfrom
{
	...
	function update($name,$url,$orderby,$ids)
	{
		$ids=is_array($ids)?array_map('intval',$ids):array(intval($ids));
		if(!$ids)return false;
		foreach($ids as $id)
		{
			if(trim($name[$id]))$this->db->update($this->table,array('name'=>trim($name[$id]),'url'=>trim($url[$id]),'orderby'=>intval($orderby[$id])),'id='.intval($id));
		}
		$this->c->cache_copyfrom();
		return true;
	}
