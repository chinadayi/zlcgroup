...
<div id="wrap">
	...
	<table cellspacing="0" class="sub" width="100%">
		...
		<tr>
			...
			<td>
			<input type="radio" name="info[adminonly]" value="1" class="radio" <?php echo $r['adminonly']?' checked="checked"':'';?> /> 启用  <input type="radio" name="info[adminonly]" value="0" class="radio" <?php echo !$r['adminonly']?' checked="checked"':'';?>  /> 关闭
			<span>启用后,该字段仅管理员可访问!</span>
			</td>
		</tr>
		<tr class="bg2"><td></td><td><input type="button" class="button" onclick="doSubmit(this);" value="编辑字段" /> <input type="button" class="button" onclick="window.location.href='?mod=member&file=member&action=fields_manage&id=<?php echo $modelid;?>'" value="取消返回" /></td></tr>
	</table>
	</form>
