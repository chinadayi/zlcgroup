...
<div id="wrap">
	<div class="tab">
		<ul>
			...
			<li><a href="?mod=member&file=member&action=fields_add&modelid=<?php echo $id;?>"><?php echo $lang['LEFT-MODULE-14'];?></a></li>	
		</ul>
	</div>
	<div class="main">
		<fieldset>
		...
		<tr>
			...
			<td align="center"><?php echo $val['disabled']?'<font color="#999999">已禁用</font> <a href="'.ADMIN_FILE.'?mod=member&file=member&action=fields_disabled&disabled=0&id='.$val['id'].'"><u>启用</u></a>':'已启用 <a href="'.ADMIN_FILE.'?mod=member&file=member&action=fields_disabled&disabled=1&id='.$val['id'].'"><u>禁用</u></a>';?></td>
			...
			<td align="center"><?php if($val['cantdelete']){echo '<font color="#999999"><u>删除</u></font> ';}else{?><a href="<?php echo ADMIN_FILE;?>?mod=member&file=member&action=fields_delete&id=<?php echo $val['id'];?>" onclick="if(!confirm('确实要删除字段 <?php echo $val['name'];?> 吗?')){return false;}"><u>删除</u></a> <?php }?><a href="<?php echo ADMIN_FILE;?>?mod=member&file=member&action=fields_edit&id=<?php echo $val['id'];?>&modelid=<?php echo $val['modelid'];?>"><u>修改</u></a></td>
		</tr>
		<?php }?>
		<tr>
		...
		<?php }else{?>
		<tr><td colspan="7">目前该模型尚未添加任何自定义字段...</td></tr>	
		<?php }?>
		<tr>
			<td colspan="7"><?php echo $member->pagestring;?></td>
		</tr>
	  </table>
	</div>
</div>
