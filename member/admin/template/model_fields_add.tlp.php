...
<div id="wrap">
	<div class="tab">
		<ul>
			<li><a href="?mod=member&file=member&action=fields_manage&id=<?php echo $modelid;?>"><?php echo $lang['LEFT-MODULE-13'];?></a></li>
	...
	<table cellspacing="0" class="sub" width="100%">
		...
		<tr>
			...
			<td>
			<input type="text" name="info[enname]"  id="enname" onblur="$.get('?mod=member&file=member&action=chkfieldname&modelid=<?php echo $modelid;?>',{data:this.value},function(data){if(data=='yes'){alert('该字段名已经存在，请更换!');$('#enname').val('');$('#enname').focus();}});" datatype="table" class="input" size="16" />
		...
		<tr>
			...
			<ul>
				...
				<li><input type="radio" class="radio" name="info[form]" value="author" />文章作者(author)</li>
			  	<li><input type="radio" class="radio" name="info[form]" value="fckeditor" />编 辑 器(fckeditor)</li>
		...
		<tr>
			<td width="190" align="right">仅后台可用：</td>
			<td>
			<input type="radio" name="info[adminonly]" value="1" class="radio" /> 启用  <input type="radio" name="info[adminonly]" value="0" class="radio" checked="checked" /> 关闭
			<span>启用后,该字段仅管理员可访问!</span>
			</td>
		</tr>
		<tr class="bg2"><td></td><td><input type="button" class="button" onclick="doSubmit(this);" value="添加字段" /> <input type="button" class="button" onclick="window.location.href='?mod=member&file=member&action=fields_manage&id=<?php echo $modelid;?>'" value="取消返回" /></td></tr>
	</table>
	</form>
