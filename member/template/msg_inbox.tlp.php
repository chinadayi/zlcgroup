...
<li class="active"><a href="member/index.php?file=msg&action=inbox"><?php echo $memlang['receive-box'];?></a></li>
...
<td class="sender"><?php echo $memlang['msg-sent-user'];?></td>
...
<tbody>
<tr>
<td class="select"><input type="checkbox" name="id[]" <?php echo $val['send_to_user']=='#system#'?'disabled="disabled"':'';?> value="<?php echo $val['id'];?>" /></td>
<td class="infotitle"><a href="member/index.php?file=<?php echo $file;?>&action=read&id=<?php echo $val['id'];?>"><?php echo $val['status']?'':'<font style="font-weight:bold;color:#999999">';?><?php echo $val['subject'];?><?php echo $val['status']?'':'</font>';?></a></td>
<td class="sender"><a href="member/index.php?file=msg&action=send&sendtouser=<?php echo $val['send_from_user'];?>" title="<?php echo $val['send_from_user'];?><?php echo $memlang['msg-reply'];?>"><?php echo $val['send_from_user'];?></a></td>
...
<td class="infotitle"><INPUT class="input_sub" id="button1" type="button" value="<?php echo $memlang['batch'].$memlang['delete'];?>" onClick="if(confirm('<?php echo $memlang['confirm'];?>')){this.form.action='member/index.php?file=<?php echo $file;?>&action=delete';this.form.submit();}" name="dodelete">  <?php echo $memlang['msg-read-tips'];?></td><td><div align="right">
</div></td>
</tr>
<tr>
